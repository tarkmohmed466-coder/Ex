...
fun CameraPassthroughView(
  ...
  if (displayMode == DisplayMode.OBJECT) {
    // In Object mode, studio background is clean dark slate
    Box(
      modifier = modifier
        .fillMaxSize()
  ...
  DisposableEffect(lifecycleOwner, displayMode) {
    ...
    cameraProviderFuture.addListener({
      try {
        ...
        Log.e("CameraPassthroughView", "Failed binding CameraX lifecycle: ${e.message}", e)
      }
    }, executor)
    ...
    onDispose {
      try {
        ...
        Log.w("CameraPassthroughView", "Error unbinding camera: ${e.message}")
      }
    }
  }
